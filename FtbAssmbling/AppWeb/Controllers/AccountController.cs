...
using ftd.mvc.Filters;
using ftd.mvc.Models;
using ftd.service;
using ftd.web;
...
namespace ftd.mvc.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            bool isValidate = AppLoginService.Instance.accountValidate(model.UserName, model.Password, ref loginUserId, out errorMsg);
            ...
            AppLoginService.Instance.login(loginUserId, "");
            AppUserSession.Instance.loginUser(loginUserId);
            return RedirectToLocal(returnUrl);
        ...
        [AllowAnonymous]
        public ActionResult MobileLogin(string loginId, string loginPwd)
        {
            string loginUserId = string.Empty;
            string errorMsg = string.Empty;

            if (loginId == null || "".equalIgnoreCase(loginId.Trim()))
            {
                return null;
            }

            if (loginPwd == null || "".equalIgnoreCase(loginPwd.Trim()))
            {
                return null;
            }

            bool isValidate = AppLoginService.Instance.accountValidate(loginId, loginPwd, ref loginUserId, out errorMsg);
            if (isValidate)
            {
                var jsonData = new
                {
        ...
        {
            //WebSecurity.Logout();
            //AppUserSession.Instance.logoutUser();
            AppLoginService.Instance.logout();
