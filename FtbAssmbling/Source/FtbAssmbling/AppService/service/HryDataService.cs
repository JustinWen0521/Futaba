...
namespace ftd.service
{
    ...
    public class HryDataService : FtdServiceBase
    {
        ...
        public String[,] getArrayForAssemblingDetail(ref String[] arrTimeWork)
        {
            ...
            {
                int outNum = 0;
                arrMCCode[i, 0] = arrTimeWork[i];//Index

                if (int.TryParse(arrTimeWork[i], out outNum))
                {
                    arrMCCode[i, 1] = arrTimeWork[i] + ":30";
                }
                ...
                arrMCCode[i,2] = null;//ITEM
                arrMCCode[i,3] = null;//QTY
            }
